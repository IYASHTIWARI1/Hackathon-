...
import { Link } from "react-router-dom";
 ...
 function Button({children,className}){
    return(
        <>
        <Link to="/CheckMate/InputPage" className={"bg-purple-700   inline-block w-40 shadow-md shadow-purple-700/50 text-white p-2 rounded-md font-medium cursor-pointer"+"  " + className}>{children}</Link>
        </>
    )
 }
